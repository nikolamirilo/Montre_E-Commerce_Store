...
const Blog = ({ params }: { params: { id: string } }) => {
  ...
  if (singleBlog)
    return (
      <div className="container max-w-[1200px] flex flex-col justify-center items-center gap-5">
        <div className="flex flex-row gap-3">
          <div className="flex flex-col justify-center gap-5">
            <h1 className="text-3xl font-bold">{singleBlog.title}</h1>
            <h2 className="text-2xl">{singleBlog.subtitle}</h2>
                  ...
                  {item.subheading && (
                    <h3
                      className={`font-semibold w-full text-xl ${
                        !item.text ? "text-center" : idx % 2 == 0 ? "text-right" : "text-left"
                      } ${!item.image ? "!text-left w-6/12" : ""}`}>
                      {item.subheading}
                    </h3>
                  )}
                  {item.text && (
                    <p
                      className={`${idx % 2 == 0 ? "text-right" : "text-left"} ${
                        !item.image ? "!text-left w-6/12" : ""
                      }`}>
                      {item.text}
                    </p>
